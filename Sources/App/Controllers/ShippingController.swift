...
final class ShippingController: ResourceRepresentable {
    ...
    func create(_ request: Request) throws -> ResponseRepresentable {
        var shipping: Shipping = try request.extractModel()
        try shipping.save()
        return shipping
...
//        return try Response(status: .created, json: shipping.makeJSON())
    ...
    func modify(_ request: Request, shipping: Shipping) throws -> ResponseRepresentable {
        var shipping: Shipping = try request.patchModel(shipping)
        try shipping.save()
        return try Response(status: .ok, json: shipping.makeJSON())
    }
